...
class Equalizer {
  ...
  drawCustomStuff() {
    ...
    for (var i = 0; i < 10; i++) {
      ...
      ctx.beginPath();
      ctx.arc(
        Math.round(left + i * space),
        (bottom - top) / 2 + this.option.pointRadius + weight,
        weight,
        0,
        fullRadian(),
        false,
  ...
  updateTooltip(e, newValue) {
    ...
    let shiftUp = ttRect.height / 2;
    let y = e.y;
    if (y <= this.baseTop - ttRect.height / 2) {
      y = this.baseTop - ttRect.height / 2;
    }
    if (y >= this.baseBottom - ttRect.height / 2) {
      y = this.baseBottom - ttRect.height / 2;
    }
