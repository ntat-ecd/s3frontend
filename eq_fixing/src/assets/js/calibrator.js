...
class Calibrator {
  ...
  drawPoint(index = 0, d = this.opt.dynamData[0], color = '#666') {
    ...
    for (var x = 1; x <= 13; x++) {
      ...
      if (
        (this.eventPoint >= 0 && index == this.eventPoint && this.toggleAnimation) ||
  ...
  getEventPoint(e) {
    ...
    if (Math.pow(x, 2) + Math.pow(y, 2) <= Math.pow(this.opt.ringRadius - this.opt.pointRadius, 2)) {
      ...
      for (var i = 0; i <= 6; i++) {
        ...
        if (
          isBetween(x, pointEdgeX - this.opt.pointRadius, pointEdgeX + this.opt.pointRadius) &&
  ...
  getEventIndex(e) {
    ...
    if (Math.pow(x, 2) + Math.pow(y, 2) <= Math.pow(this.opt.ringRadius - this.opt.pointRadius, 2)) {
      ...
      for (var i = 0; i <= 6; i++) {
        if (i != 4) {
          if (eventDegree <= this.opt.dataRange[i].max && eventDegree >= this.opt.dataRange[i].min) {
            hoverArea = i;
          }
        } else {
  ...
  spitData() {
    var data = [];
    for (var i = 0; i < 7; i++) {
      data[i] = this.opt.dynamData[i] + 90 >= 180 ? this.opt.dynamData[i] + 90 - 360 : this.opt.dynamData[i] + 90;
    }
    return data;
  }
  printData() {
...
var handleTest = function () {
  if (!testStart && document.getElementById('cbTest').classList.contains('stop')) {
    document.getElementById('cbTest').classList.remove('stop');
